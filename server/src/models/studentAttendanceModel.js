...
const getAttendanceData = async (student_id) => {
    ...
    total_attendance AS (
        SELECT
            a.subject_id,
            COUNT(CASE WHEN a.attendance = true THEN 1 END) AS attended_lecture  -- Counting true values for attended_lecture
        FROM attendance a
    ...
    LEFT JOIN total_attendance ta ON sl.subject_id = ta.subject_id
