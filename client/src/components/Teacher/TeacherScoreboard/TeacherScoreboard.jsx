...
const TeacherScoreboard = () => {
    ...
    if (token) {
      const fetchSubjectsAndExams = async () => {
        try {
          ...
          const subjectResponse = await fetch('http://192.168.29.80:3000/api/teachers/subjects', {
            headers: {
          ...
          // Fetch exams
          const examResponse = await fetch('http://192.168.29.80:3000/api/admin/exams', {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
  ...
  const handleSubjectChange = async (subject) => {
    ...
    try {
      // Fetch students based on the selected subject
      const studentResponse = await fetch(`http://192.168.29.80:3000/api/teachers/subject-students?subjectCode=${subject}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    const fetchMarksData = async () => {
      ...
      try {
        const marksResponse = await fetch(
          `http://192.168.29.80:3000/api/teachers/marks?subjectCode=${selectedSubject}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleSaveMarks = async () => {
    ...
    try {
      const url = isUpdateMode
        ? 'http://192.168.29.80:3000/api/teachers/marks/update' // Use PUT for updating marks
        : 'http://192.168.29.80:3000/api/teachers/marks/upload'; // Use POST for saving marks
