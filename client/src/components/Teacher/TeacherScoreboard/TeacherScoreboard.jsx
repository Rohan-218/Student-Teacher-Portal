...
const TeacherScoreboard = () => {
    ...
    if (token) {
      const fetchSubjectsAndExams = async () => {
        try {
          ...
          const subjectResponse = await fetch('http://student-teacher-portal-server.onrender.com/api/teachers/subjects', {
            headers: {
          ...
          // Fetch exams
          const examResponse = await fetch('http://student-teacher-portal-server.onrender.com/api/teachers/exams', {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
  ...
  const handleSubjectChange = async (subject) => {
    ...
    try {
      // Fetch students based on the selected subject
      const studentResponse = await fetch(`http://student-teacher-portal-server.onrender.com/api/teachers/subject-students?subjectCode=${subject}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    const fetchMarksData = async () => {
      ...
      try {
        const marksResponse = await fetch(
          `http://student-teacher-portal-server.onrender.com/api/teachers/marks?subjectCode=${selectedSubject}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleSaveMarks = async () => {
    ...
    try {
      const url = isUpdateMode
        ? 'http://student-teacher-portal-server.onrender.com/api/teachers/marks/update' // Use PUT for updating marks
        : 'http://student-teacher-portal-server.onrender.com/api/teachers/marks/upload'; // Use POST for saving marks
