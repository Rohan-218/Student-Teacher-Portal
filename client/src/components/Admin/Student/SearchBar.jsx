...
const SearchBar = ({ onSearch, fetchInitialStudents }) => {
  ...
  const handleSearch = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://192.168.29.80:3000/api/admin/students/search?name=${searchName}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const fetchStudentsByBranchAndSemester = async (branch, semester) => {
    if (branch && semester) {
      try {
        const token = localStorage.getItem('token');
        const res = await fetch(`http://192.168.29.80:3000/api/admin/students/branch-semester?branch_name=${branch}&semester=${semester}`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
