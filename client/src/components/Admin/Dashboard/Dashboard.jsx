   ...
   useEffect(() => {
    const fetchCounts = async () => {
      try {
        ...
        const studentRes = await fetch('http://student-teacher-portal-server.onrender.com/api/admin/students/count', { headers });
        if (!studentRes.ok) throw new Error(`Failed to fetch: ${studentRes.statusText}`);
        const studentData = await studentRes.json();
        setStudentCount(studentData.studentCount);
  
        const teacherRes = await fetch('http://student-teacher-portal-server.onrender.com/api/admin/teachers/count', { headers });
        if (!teacherRes.ok) throw new Error(`Failed to fetch: ${teacherRes.statusText}`);
        const teacherData = await teacherRes.json();
        setTeacherCount(teacherData.teacherCount);
  
        const branchRes = await fetch('http://student-teacher-portal-server.onrender.com/api/admin/branches/count', { headers });
        if (!branchRes.ok) throw new Error(`Failed to fetch: ${branchRes.statusText}`);
        const branchData = await branchRes.json();
        setBranchCount(branchData.branchCount);
  
        const subjectRes = await fetch('http://student-teacher-portal-server.onrender.com/api/admin/subjects/count', { headers });
    ...
    const fetchBranchStudentCounts = async () => {
      try {
        ...
        const res = await fetch('http://student-teacher-portal-server.onrender.com/api/admin/branches/student-count', {
          method: 'GET',
          headers: {
