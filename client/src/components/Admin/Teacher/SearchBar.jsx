...
const SearchBar = ({ onSearch, fetchInitialTeachers }) => {
  ...
  useEffect(() => {
    const handleSearch = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const res = await fetch(`http://student-teacher-portal-server.onrender.com/api/admin/teachers/search?name=${searchName}`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
  ...
  const fetchTeachersByBranchAndSemester = async (branch, semester) => {
    if (branch && semester) {
      try {
        const token = localStorage.getItem('token');
        const res = await fetch(`http://student-teacher-portal-server.onrender.com/api/admin/teachers/branch-semester?branchName=${branch}&semester=${semester}`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
